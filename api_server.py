...
async def analyze_stock(req: AnalyzeReq, username: str = Depends(verify_token), background_tasks: BackgroundTasks = None):
    ...
    # 参数校验
    # 根据市场类型验证股票代码格式
    market_type = req.market_type or (req.parameters or {}).get("market_type") or (req.parameters or {}).get("market", "A股")
    if not symbol:
...
async def analysis_single(req: AnalyzeReq, background_tasks: BackgroundTasks, username: str = Depends(verify_token)):
    ...
    # 转换中文名称为英文
    raw_selected = req.selected_analysts or []
    selected = [analyst_name_map.get(name, name) for name in raw_selected] if raw_selected else ["market", "news", "fundamentals"]
